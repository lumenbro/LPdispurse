...
from stellar_sdk import Keypair, Asset, Payment
...
async def calculate_referral_shares(db_pool_copytrading, user_id, fee):
    async with db_pool_copytrading.acquire() as conn:
...
async def export_unpaid_rewards(db_pool_nitro, db_pool_copytrading, output_file):
    """
    Export unpaid rewards to a CSV file if the total amount per user is >= 0.1 XLM.
    
    Args:
        db_pool_nitro: Database pool for the NITRO Secure Enclave database (stellar_bot).
        db_pool_copytrading: Database pool for the Copy Trading/Referral database (copytrading).
    ...
    async with db_pool_copytrading.acquire() as conn:
        ...
        total_payout = 0
        payout_list = []
        async with db_pool_nitro.acquire() as conn_nitro:
            for row in rewards:
                user_id = row['user_id']
                amount = float(row['total_amount'])  # Convert Decimal to float
                public_key = await conn_nitro.fetchval(
...
async def daily_payout(db_pool_nitro, db_pool_copytrading, bot, chat_id, app_context):
    output_file = f"referral_rewards_export_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.csv"
    exported_file, total_payout, payout_list = await export_unpaid_rewards(db_pool_nitro, db_pool_copytrading, output_file)
    ...
    try:
        disbursement_public_key = await app_context.load_public_key(app_context.fee_telegram_id)
