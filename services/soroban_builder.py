...
async def build_and_submit_soroban_transaction(telegram_id, soroban_ops, app_context, original_tx_hash, trader_wallet, use_rpc=False):
    ...
    async with client:
        ...
        try:
            ...
            # Fee calculation
            xlm_balance = float(next((b["balance"] for b in account_data["balances"] if b["asset_type"] == "native"), "0"))
            fee_percentage = 0.009 if await has_referrer(telegram_id, app_context.db_pool_copytrading) else 0.01
...
async def try_sdex_fallback(telegram_id, tx, trader_wallet, chat_id, app_context):
    ...
    try:
        ...
        await log_xlm_volume(telegram_id, response["xlm_volume"], swap_hash, app_context.db_pool_copytrading)
        await calculate_referral_shares(app_context.db_pool_copytrading, telegram_id, fee_amount)
