                    ...
                    if soroban_ops:
                        ...
                        if response:
                            ...
                            message = (
                                ...
                                f"Service Fee: {response['service_fee']:.7f} XLM\n"
                                f"Tx: {response['hash']}"
                            )
                            await self.app_context.bot.send_message(chat_id, message)
                        else:
    ...
    async def start_streaming(self, chat_id, telegram_id):
        async with self.app_context.stream_lock:
            if chat_id in self.tasks and any(not t.done() for t in self.tasks[chat_id].values()):
                ...
                return False
            async with self.app_context.db_pool_copytrading.acquire() as conn:
                try:
